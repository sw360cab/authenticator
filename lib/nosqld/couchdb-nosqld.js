...
Couchd.prototype.updateUser = function (username,dataToUpdate,callback) {
  ...
  this._query({username:username},function(err,doc) {
    ...
    if (dataToUpdate.oldPassword){
      ...
      if (!currentPassword || self.options.passwordCrypto(dataToUpdate.oldPassword) !== currentPassword ) {
         // wrong password
         return callback({msg:"wrong password"});
      }
      // remove old password from fields
      delete dataToUpdate.oldPassword;
      // encrypt new password
      dataToUpdate.password = self.options.passwordCrypto(dataToUpdate.password);
    }
